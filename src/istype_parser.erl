...
-export([parse_record/2, parse_records/1, parse_records/2, parse_records/3, parse_type/2, parse_types/1, parse_types/2, resolve_type/2,
	 resolve_types/1]).
...
%% @end
parse_types(Forms) -> parse_types(Forms, #{}).

-spec parse_types(istype:forms() | module(), istype:types()) -> istype:types().

parse_types(Module, Types) when is_atom(Module) -> parse_types(forms:read(Module), Types);
parse_types(Forms, Types) ->
    {attribute, _, module, Module} = lists:keyfind(module, 3, Forms),
    ParsedTypes = lists:foldl(fun ({attribute, _, Class, {TypeLabel, _, TypeParams}} = Form, Acc0)
				      when Class =:= type orelse Class =:= opaque ->
				      {Type, Acc1} = parse_type(Module, Form, Acc0), Key = {Module, TypeLabel, length(TypeParams)}, Acc1#{Key => Type};
				  (_, Acc) -> Acc
			      end,
			      Types, Forms),
    ParsedTypes.
...
%% parse_type_list
%%=========================================================
-spec parse_type_list(module(), istype:forms(), istype:types()) -> {[istype:type()], istype:types()}.

parse_type_list(Module, Forms, Types) -> parse_type_list(fun parse_type/3, Module, Forms, Types).

parse_type_list(Fun, Module, Forms, Types) ->
    lists:foldr(fun (Form, {TypeAcc, ExtTypeAcc0}) -> {Type, ExtTypeAcc1} = Fun(Module, Form, ExtTypeAcc0), {[Type | TypeAcc], ExtTypeAcc1} end,
		{[], Types}, Forms).
...
%%      into the internal type format.
%% @end
parse_type(Module, Form) -> {Type, _} = parse_type(Module, Form, #{}), Type.
...
parse_type(Module, Form, Types) -> do_parse_type(Module, Form, Types).
...
%% @end
%%======================================
do_parse_type(Module, {attribute, _, Class0, {_, {remote_type, _, _} = TypeSpec, TypeParams}}, Types0)
    when Class0 =:= type orelse Class0 =:= opaque ->
    {Type, Types1} = parse_type(Module, TypeSpec, Types0), {Type#type{params = TypeParams}, Types1};
do_parse_type(Module, {attribute, _, Class0, {_, TypeSpec, TypeParams}}, Types0) when Class0 =:= type orelse Class0 =:= opaque ->
    ...
    {case Type of
       #type{} -> Type#type{params = TypeParams};
       #literal{} -> Type
     end,
     Types1};
%%======================================
...
%% @end
%%======================================
do_parse_type(Module, {var, _, '_'}, Types) -> parse_type(Module, {type, 1, any, []}, Types);
%%======================================
...
%% @end
%%======================================
do_parse_type(_, {var, _, _} = Var, Types) -> {Var, Types};
%%======================================
...
%% @end
%%======================================
do_parse_type(Module, {ann_type, _, [_, Type]}, Types) -> parse_type(Module, Type, Types);
%%======================================
%% any()
%%======================================
...
%%      a literal.
%% @end
do_parse_type(_, {nil, _} = Nil, Types) -> {#literal{value = Nil}, Types};
do_parse_type(_, {type, Line, nil, []}, Types) -> {#literal{value = {nil, Line}}, Types};
do_parse_type(_, {call, Line, {atom, _, nil}, []}, Types) -> {#literal{value = {nil, Line}}, Types};
%%======================================
%% Atom
%%======================================
%% atom()
...
%% @end
do_parse_type(_, {atom, _, _} = Atom, Types) -> {#literal{value = Atom}, Types};
%%======================================
%% Bitstring
%%======================================
...
%% @end
do_parse_type(_, {type, _, binary, [{integer, _, M}, {integer, _, N}]}, Types) -> {#type{type = bitstring, spec = {M, N}}, Types};
do_parse_type(_, {bin, _, []}, Types) -> {#type{type = bitstring, spec = {0, 0}}, Types};
%%======================================
%% float()
%%======================================
...
%% @end
do_parse_type(_, {type, _, 'fun', []}, Types) -> {#type{type = 'fun', spec = any}, Types};
do_parse_type(_, {call, _, {atom, _, 'fun'}, []}, Types) -> {#type{type = 'fun', spec = any}, Types};
do_parse_type(Module, {type, _, 'fun', [{type, _, any}, ReturnType0]}, Types0) ->
    {ReturnType1, Types1} = parse_type(Module, ReturnType0, Types0), {#type{type = 'fun', spec = {any, ReturnType1}}, Types1};
do_parse_type(Module, {type, _, 'fun', [{type, _, product, ParameterTypes0}, ReturnType0]}, Types0) ->
    ...
    {#type{type = 'fun', spec = {ParameterTypes1, ReturnType1}}, Types2};
%%======================================
%% Integer
%%======================================
%% integer()
...
%% @end
do_parse_type(_, {integer, _, _} = Integer, Types) -> {#literal{value = Integer}, Types};
do_parse_type(_, {op, _, '-', {integer, _, _}} = Integer, Types) -> {#literal{value = Integer}, Types};
%%==================
...
do_parse_type(_, {type, _, range, [Low, High]}, Types) ->
    {#type{type = range, spec = {#literal{value = Low}, #literal{value = High}}}, Types};
%%======================================
%% List
%%======================================
...
%% @end
do_parse_type(_, {type, _, list, [{type, _, any, []}]}, Types) -> {#type{type = list, spec = any}, Types};
do_parse_type(Module, {type, _, list, [ValueType0]}, Types0) ->
    {ValueType1, Types1} = parse_type(Module, ValueType0, Types0),
    {#type{type = list, spec = {maybe_empty, ValueType1, parse_type(Module, {type, 1, nil, []})}}, Types1};
do_parse_type(Module, {type, _, maybe_improper_list, [_, _] = ValueTypes0}, Types0) ->
    ...
    {#type{type = list, spec = list_to_tuple([maybe_empty | ValueTypes1])}, Types1};
do_parse_type(Module, {type, _, nonempty_improper_list, ValueTypes0}, Types0) ->
    {ValueTypes1, Types1} = parse_type_list(Module, ValueTypes0, Types0),
    {#type{type = list, spec = list_to_tuple([nonempty | ValueTypes1])}, Types1};
do_parse_type(Module, {type, _, nonempty_list, [ValueType0]}, Types0) ->
    {ValueType1, Types1} = parse_type(Module, ValueType0, Types0),
    {#type{type = list, spec = {nonempty, ValueType1, parse_type(Module, {type, 1, nil, []})}}, Types1};
%%======================================
%% Map
%%======================================
...
%% @end
do_parse_type(_, {map, _, []}, Types) -> {#type{type = map, spec = empty}, Types};
do_parse_type(_, {type, _, map, any}, Types) -> {#type{type = map, spec = any}, Types};
do_parse_type(_, {call, _, {atom, _, map}, []}, Types) -> {#type{type = map, spec = any}, Types};
do_parse_type(_, {type, _, map, []}, Types) -> {#type{type = map, spec = empty}, Types};
do_parse_type(Module, {type, _, map, MapFields}, Types0) ->
    {MapFieldTypes, Types1} = parse_map_fields(Module, MapFields, Types0), {#type{type = map, spec = MapFieldTypes}, Types1};
%%======================================
%% Tuple
%%======================================
...
%% @end
do_parse_type(_, {tuple, _, []}, Types) -> {#type{type = tuple, spec = empty}, Types};
do_parse_type(Module, {tuple, _, FieldTypes0}, Types0) ->
    {FieldTypes1, Types1} = parse_tuple_fields(Module, FieldTypes0, Types0), {#type{type = tuple, spec = FieldTypes1}, Types1};
do_parse_type(_, {type, _, tuple, any}, Types) -> {#type{type = tuple, spec = any}, Types};
do_parse_type(_, {call, _, {atom, _, tuple}, []}, Types) -> {#type{type = tuple, spec = any}, Types};
do_parse_type(_, {type, _, tuple, []}, Types) -> {#type{type = tuple, spec = empty}, Types};
...
do_parse_type(Module, {type, _, union, UnionTypes0}, Types0) ->
    {UnionTypes1, Types1} = parse_type_list(Module, UnionTypes0, Types0), {#type{type = union, spec = UnionTypes1}, Types1};
%%======================================
%% term()
%%======================================
...
%% @end
do_parse_type(Module, {type, Line, term, []}, Types) -> parse_type(Module, {type, Line, any, []}, Types);
%%======================================
%% binary()
%%======================================
...
do_parse_type(Module, {type, Line, byte, []}, Types) ->
    parse_type(Module, {type, Line, range, [{integer, Line, 0}, {integer, Line, 255}]}, Types);
%%======================================
%% char()
%%======================================
...
do_parse_type(Module, {type, Line, char, []}, Types) ->
    parse_type(Module, {type, Line, range, [{integer, Line, 0}, {integer, Line, 1114111}]}, Types);
%%======================================
%% nil()
%%======================================
%% @doc Alias for [].
...
%% @end
do_parse_type(Module, {type, Line, list, []}, Types) -> parse_type(Module, {type, Line, list, [{type, Line, any, []}]}, Types);
%%======================================
%% maybe_improper_list()
%%======================================
...
%%      Calls handled by the default call handler.
%% @end
do_parse_type(Module, {type, Line, string, []}, Types) -> parse_type(Module, {type, Line, list, [{type, Line, char, []}]}, Types);
%%======================================
%% nonempty_string()
%%======================================
...
do_parse_type(Module, {type, Line, iodata, []}, Types) ->
    parse_type(Module, {type, Line, union, [{type, Line, iolist, []}, {type, Line, binary, []}]}, Types);
%%======================================
%% iolist()
%%======================================
...
%%      Calls handled by the default call handler.
%% @end
do_parse_type(_, {type, _, iolist, []}, Types) -> {#type{type = iolist}, Types};
do_parse_type(_, {type, iolist, []}, Types) -> {#type{type = iolist}, Types};
%%======================================
%% function()
%%======================================
...
%%      Alias for fun().
%% @end
do_parse_type(Module, {type, Line, function, []}, Types) -> parse_type(Module, {type, Line, 'fun', []}, Types);
do_parse_type(Module, {call, Line, {atom, _, function}, []}, Types) -> parse_type(Module, {call, Line, {atom, Line, 'fun'}, []}, Types);
%%======================================
%% module()
%%======================================
...
%%      Calls handled by the default call handler.
%% @end
do_parse_type(Module, {type, Line, module, []}, Types) -> parse_type(Module, {type, Line, atom, []}, Types);
%%======================================
%% mfa()
%%======================================
...
do_parse_type(Module, {type, Line, mfa, []}, Types) ->
    parse_type(Module, {type, Line, tuple, [{type, Line, module, []}, {type, Line, atom, []}, {type, Line, arity, []}]}, Types);
%%======================================
%% arity()
%%======================================
...
do_parse_type(Module, {type, Line, identifier, []}, Types) ->
    parse_type(Module, {type, Line, union, [{type, Line, pid, []}, {type, Line, port, []}, {type, Line, reference, []}]}, Types);
%%======================================
%% node()
%%======================================
...
%%      Calls handled by the default call handler.
%% @end
do_parse_type(Module, {type, Line, node, []}, Types) -> parse_type(Module, {type, Line, atom, []}, Types);
%%======================================
%% timeout()
%%======================================
...
do_parse_type(Module, {type, Line, timeout, []}, Types) ->
    parse_type(Module, {type, Line, union, [{atom, Line, infinity}, {type, Line, non_neg_integer, []}]}, Types);
%%======================================
%% no_return()
%%======================================
...
%%      Calls handled by the default call handler.
%% @end
do_parse_type(Module, {type, Line, no_return, []}, Types) -> parse_type(Module, {type, Line, none, []}, Types);
%%======================================
%% non_neg_integer()
%%======================================
...
do_parse_type(Module, {type, Line, non_neg_integer, []}, Types) ->
    parse_type(Module, {type, Line, range, [{integer, Line, 0}, {atom, Line, undefined}]}, Types);
%%======================================
%% pos_integer()
%%======================================
...
do_parse_type(Module, {type, Line, pos_integer, []}, Types) ->
    parse_type(Module, {type, Line, range, [{integer, Line, 1}, {atom, Line, undefined}]}, Types);
%%======================================
%% neg_integer()
%%======================================
...
do_parse_type(Module, {type, Line, neg_integer, []}, Types) ->
    parse_type(Module, {type, Line, range, [{atom, Line, undefined}, {op, Line, '-', {integer, Line, 1}}]}, Types);
%%======================================
...
do_parse_type(Module, {type, Line, nonempty_maybe_improper_list, []}, Types) ->
    parse_type(Module, {type, Line, nonempty_maybe_improper_list, [{type, Line, any, []}, {type, Line, any, []}]}, Types);
%%======================================
...
do_parse_type(Module, {type, _, record, [{atom, _, Record} | RecordFields0]}, Types0) ->
    ParseFun = fun (M, {type, _, field_type, [{atom, _, Field}, FieldType0]}, TypesAcc0) ->
		       {FieldType1, TypesAcc1} = parse_type(M, FieldType0, TypesAcc0), {{Field, FieldType1}, TypesAcc1}
	       end,
    {RecordFields1, Types1} = parse_type_list(ParseFun, Module, RecordFields0, Types0),
    {#type{type = record, spec = {Record, RecordFields1}}, Types1};
do_parse_type(Module, {record, _, Record, RecordFields}, Types) ->
    Fields = [parse_record_field(V1) || V1 <- RecordFields],
    FieldTypes = [do_parse_type_1(V2, Module, Types) || V2 <- RecordFields],
    {#type{type = record, spec = {Record, lists:zip(Fields, FieldTypes)}}, Types};
%%======================================
%% Default call handler
%%======================================
do_parse_type(Module, {call, Line, {atom, _, Type}, TypeArgs}, Types) -> parse_type(Module, {type, Line, Type, TypeArgs}, Types);
%%======================================
%% Remote call handler
%%======================================
...
do_parse_type(_, {remote_type, _, RemoteTypeSpec} = TypeSpec, Types0) ->
    [{atom, _, Module}, {atom, _, Type}, TypeParams] = RemoteTypeSpec,
    TypeKey = {Module, Type, length(TypeParams)},
    ParsedKey = {parsed, Module},
    case Types0 of
      #{TypeKey := RemoteType} -> {RemoteType, Types0};
      #{ParsedKey := true} -> parse_error({type_not_found, TypeKey});
      _ -> Types1 = parse_types(Module), Types2 = maps:merge(Types0#{ParsedKey => true}, Types1), parse_type(Module, TypeSpec, Types2)
    end;
%%======================================
%% Default handlers
%%======================================
do_parse_type(Module, {user_type, _, Type, TypeArgs}, Types0) ->
    {TList, Types1} = parse_type_list(Module, TypeArgs, Types0), {#type{module = Module, type = Type, spec = TList}, Types1};
do_parse_type(Module0, {type, _, Type, TypeArgs}, Types0) ->
    ...
    {#type{module = Module1, type = Type, spec = TList}, Types1};
do_parse_type(_, Type, _) -> parse_error({unparsed, Type}).

do_parse_type_1({record_field, _, _, Type0}, Module, Types) -> {Type1, _} = parse_type(Module, Type0, Types), Type1.

%%=========================================================
%% parse_error
%%=========================================================
parse_error(Type) -> error({parse_type, Type}).
...
parse_map_fields(Module, Fields0, Types0) ->
    {Exact, Types1} = lists:foldr(fun (Types, {FieldAcc, TypeAcc0}) ->
					  {Fields, TypeAcc1} = parse_type_list(Module, Types, TypeAcc0), {[list_to_tuple(Fields) | FieldAcc], TypeAcc1}
				  end,
				  {[], Types0}, [F || {type, _, map_field_exact, F} <- Fields0]),
    {Assoc, Types2} = lists:foldr(fun (Types, {FieldAcc, TypeAcc0}) ->
					  {Fields, TypeAcc1} = parse_type_list(Module, Types, TypeAcc0), {[list_to_tuple(Fields) | FieldAcc], TypeAcc1}
				  end,
				  {[], Types1}, [F || {type, _, map_field_assoc, F} <- Fields0]),
    {{Exact, Assoc}, Types2}.
...
parse_records(Forms) -> {_, Records} = parse_records(Forms, #{}, #{}), Records.

-spec parse_records(istype:forms(), istype:types()) -> {istype:types(), istype:records()}.

parse_records(Forms, Types) -> parse_records(Forms, Types, #{}).
...
parse_records(Forms, Types, Records) ->
    ...
    lists:foldl(fun ({attribute, _, record, {RecordLabel, _}} = Form, {TypesAcc0, RecordsAcc}) ->
			{Record, TypesAcc1} = parse_record(Module, Form, TypesAcc0), {TypesAcc1, RecordsAcc#{RecordLabel => Record}};
		    (_, Acc) -> Acc
		end,
		{Types, Records}, Forms).
...
parse_record(Module, Form) -> {Record, _} = parse_record(Module, Form, #{}), Record.
...
parse_record(Module, {attribute, _, record, {Record, RecordFields}}, Types0) ->
    ...
    {#record{arity = Arity, record = Record, fields = Fields, types = FieldTypes, defaults = FieldDefaults}, Types1}.
...
parse_record_fields(Module, RecordFields, Types0) ->
    ...
    Fields = [parse_record_field(V1) || V1 <- RecordFields],
    FieldDefaults = [parse_record_field_default(V2) || V2 <- RecordFields],
    {FieldTypes, Types1} = lists:foldr(fun (RecordField, {RecordFieldAcc, TypesAcc0}) ->
				       ...
				       {[], Types0}, RecordFields),
    {{Arity, Fields, maps:from_list(lists:zip(Fields, FieldTypes)), maps:from_list(lists:zip(Fields, FieldDefaults))}, Types1}.

%% @doc Extract the name from a record field.
%% @end
parse_record_field({typed_record_field, RecordField, _}) -> parse_record_field(RecordField);
parse_record_field({record_field, _, {atom, _, RecordField}}) -> RecordField;
parse_record_field({record_field, _, {atom, _, RecordField}, _}) -> RecordField.

%% @doc Extract the default from a record field.
%% @end
parse_record_field_default({typed_record_field, RecordField, _}) -> parse_record_field_default(RecordField);
parse_record_field_default({record_field, Line, _}) -> #literal{value = {atom, Line, undefined}};
parse_record_field_default({record_field, _, _, Default}) -> #literal{value = Default}.

%% @doc Extract the type from a record field.
%% @end
parse_record_field_type(Module, {typed_record_field, _, Type}, Types) -> parse_type(Module, Type, Types);
parse_record_field_type(Module, _, Types) -> parse_type(Module, {type, 1, any, []}, Types).
...
resolve_types(Types0) ->
    Types1 = maps:fold(fun (K, #type{} = V0, Acc) -> V1 = resolve_type(V0, Types0), Acc#{K => V1#type{params = V0#type.params}};
			   (_, _, Acc) -> Acc
		       end,
		       #{}, Types0),
...
resolve_type(#type{spec = Spec} = Base, Types) when is_list(Spec) ->
    MFA = {Base#type.module, Base#type.type, length(Spec)},
    case Types of
      #{MFA := Type0} ->
	  Type1 = do_resolve_type(Base, Type0, Types),
	  case Type1 of
	    #type{} -> Type1#type{params = Base#type.params};
	    _ -> Type1
	  end;
      _ -> Base
    end;
resolve_type(Base, _) -> Base.

do_resolve_type(Base, #type{} = Type, Types) ->
    ParamKeys = [do_resolve_type_1(V1) || V1 <- Type#type.params],
    ParamMap = maps:from_list(lists:zip(ParamKeys, Base#type.spec)),
    Spec0 = istype_transform:update_term(fun Fun({var, _, Var}) -> maps:get({var, Var}, ParamMap);
					     Fun(#type{} = T) -> T#type{spec = istype_transform:update_term(Fun, T#type.spec)};
					     Fun(X) -> X
					 end,
					 Type#type.spec),
    Spec1 = istype_transform:update_term(fun (#type{} = TypeX) -> resolve_type(TypeX, Types);
					     (X) -> X
					 end,
					 Spec0),
    resolve_type(Type#type{spec = Spec1}, Types);
do_resolve_type(_, #literal{} = Literal, _) -> Literal.

do_resolve_type_1({var, _, V}) -> {var, V};
do_resolve_type_1(V) -> V.
