...
remote_test() ->
  ?assertEqual(true, istype(apple, istype_remote:type_a())).
...
invocation_test() ->
    ...
    ?assertEqual(true, istype(fun() -> atom end(), atom())),
...
guard_test() ->
    TestAtom = return_value(atom),
    ?assertEqual(true, case ?MODULE:return_value(atom) of
                           X0 when istype(X0, atom()) -> true
                       end),

    Record1 = return_value(#record_a{}),
    ?assertEqual(true, case Record1 of
                           X1 when istype(X1, #record_a{}) -> true
                       end),

    ?assertEqual(true, fun(X2) when istype(X2, atom()) -> true;
                          (_) -> false
                       end(TestAtom)).
...
pid_validation_test() ->
    Self = self(),
    ?assertEqual(false, istype(return_value(atom), pid())),
    ?assertEqual(true, istype(return_value(Self), pid())).
...
-type optional_map_key_type() :: #{atom => binary(),
                                   binary() => binary()}.
-type optional_map_value_type() :: #{binary() => atom,
                                     binary() => binary()}.
...
any_map_validation_test() ->
    ?assertEqual(false, istype(return_value(atom), map())),
    ?assertEqual(true, istype(return_value(#{}), map())).
...
optional_map_conversion_test() ->
    ...
    %% Key type is literal
    ?assertEqual(#{atom => <<"binary">>,
                   <<"binary">> => 1},
                 totype(return_value(#{<<"atom">> => <<"binary">>,
                                       <<"binary">> => 1}),
                        optional_map_key_type())),
    ...
    ?assertEqual(#{<<"binary">> => atom,
                   <<"binary">> => 1},
                 totype(return_value(#{<<"binary">> => <<"atom">>,
                                       <<"binary">> => 1}),
                        optional_map_value_type())),
...
binary_validation_test() ->
    ?assertEqual(false, istype(return_value(atom), binary())),
    ?assertEqual(true, istype(return_value(<<"binary">>), binary())).
...
bitstring_validation_test() ->
    ?assertEqual(false, istype(atom, bitstring())),
    ?assertEqual(true, istype(<<"binary">>, bitstring())).
...
byte_conversion_test() ->
    0 = totype(0, byte()),
    ?CONVERT_ERROR(atom, byte()),
    ?CONVERT_ERROR(-1, byte()).
...
char_validation_test() ->
    ...
    ?assertEqual(false, istype(return_value(16#110000), char())).
...
function_conversion_test() ->
    Fun = fun() -> ok end,
    ?CONVERT_ERROR(atom, function_type()),
    ?CONVERT_ERROR(Fun, function_type()).
...
module_validation_test() ->
    ?assertEqual(true, istype(return_value(atom), module())),
    ?assertEqual(false, istype(return_value(<<"binary">>), module())).
...
arity_conversion_test() ->
    0 = totype(0, arity()),
    ?CONVERT_ERROR(atom, arity()),
    ?CONVERT_ERROR(-1, arity()).
...
node_validation_test() ->
    ?assertEqual(true, istype(return_value(atom), node())),
    ?assertEqual(false, istype(return_value(<<"binary">>), node())).
